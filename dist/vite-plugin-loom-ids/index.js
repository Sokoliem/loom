...
function walkChildren(node, ctx, push) {
  let siblingIdx = 0;
  walkAll(node, (n2, parent) => {
    if (n2.type !== "JSXOpeningElement") return;
    if (skip(n2)) return;
    const componentName = openingNameToString(n2.name);
    const staticProps = extractStaticProps(n2);
    const spreadLoc = spreadSourceLoc(n2);
    ...
    const namePos = posOfName(n2.name);
    if (namePos !== null) {
